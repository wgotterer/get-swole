...
julietteburpees = OnlineClass.create(video:"https://youtu.be/4bqJSSLNJ3M", description:"Burpee demontration from our most beloved trainer Juliette.", trainer_id:juliette.id, category:"Upper Body", name:"Burpees with Juliette")
juliettefetch = OnlineClass.create(video:"https://youtu.be/eaUoVa08UTI", description:"Watch and learn how to fetch like Juliette.", trainer_id:juliette.id, category:"Weight Loss", name:"Fetching with Juliette")
julietteyoga = OnlineClass.create(video:'https://youtu.be/iXajQQYkuPM', description:'Learn how to form the downward dog and shavasana poses with Juliette.', trainer_id:juliette.id, category:'Stretching and Flexibility', name:'Yoga with Juliette')
brendanshadowboxing = OnlineClass.create(video:'https://youtu.be/R0JtO-Wv3MY', description:'Learn how to shadow box for cardio with Brendan', trainer_id:brendan.id, category:'Weight Loss', name:'Shadow Boxing with Brendan')
brendanrunninginplace = OnlineClass.create(video:'https://youtu.be/Zxji0k2Uds0', description:'Watch Brendan practice running in place, the easier way to run (you can watch tv while you do it!!!)', trainer_id:brendan.id, category:'Weight Loss', name:'Running with Brendan')
brendansquats = OnlineClass.create(video:' https://youtu.be/JPreuNx7hq4', description:'Brendan is back with a squat demonstration starring a special guest', trainer_id:brendan.id, category:'Lower Body', name:'Squats with Brendan')
...
ClientOnlineClass.create( online_class_id: julietteburpees.id, client_id: billy.id)
ClientOnlineClass.create( online_class_id: julietteburpees.id, client_id: mandy.id)
...
PrivateClass.create(description:'I need some help with my flexibility', date:'20211213090000', client_id:billy.id, trainer_id:jane.id)
PrivateClass.create(description:'I want to learn to walk around the block', date:'20211219120000', client_id:billy.id, trainer_id:juliette.id)
PrivateClass.create(description:'I would like to meet the king of flexibility, Wilhelm', date:'20211221120000', client_id:billy.id, trainer_id:william.id)
PrivateClass.create(description:'I would also like to meet the band man!', date:'20211223', client_id:billy.id, trainer_id:william.id)
...
PrivateClass.create(description:'I would like to learn how to do yoga!', date:'20211213120000', client_id:kyle.id, trainer_id:juliette.id)
PrivateClass.create(description:'Your fetching skills are awesome, teach me', date:'20211215020000', client_id:ivan.id, trainer_id:juliette.id)
PrivateClass.create(description:'Need some napping pointers', date:'20211225090000', client_id:xavier.id, trainer_id:juliette.id)

Review.create(content:"Juliette is the best training", rating:"10", online_class_id: julietteburpees.id, client_id: billy.id, client_name: billy.name)
